...
export default function EmailVerification() {
  ...
  useEffect(() => {
    if (!processNumber || !courtEndpoint) {
      ...
      toast({
        ...
        description: "Por favor, selecione um processo primeiro.",
        variant: "destructive",
      });
      navigate('/public/search');
    }
  }, [processNumber, courtEndpoint, navigate]);
  ...
  return (
    <div className="space-y-6">
      <div className="mb-4">
        <h2 className="text-xl font-semibold mb-2">Verificação de Identidade</h2>
        <p className="text-sm text-gray-600">
          Para acessar as informações do processo, precisamos verificar sua identidade
        </p>
      </div>
      ...
      <div className="text-xs text-gray-500 mt-8 bg-gray-50 p-4 rounded-md">
        <p className="mb-2 font-medium">Sobre a verificação:</p>
        <ul className="list-disc pl-5 space-y-1">
          <li>Seu email não será armazenado permanentemente</li>
          <li>O código de verificação é válido por 10 minutos</li>
          <li>Esta etapa ajuda a prevenir o acesso automatizado em massa</li>
        </ul>
      </div>
    </div>
  );
