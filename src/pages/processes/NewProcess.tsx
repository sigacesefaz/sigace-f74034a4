...
export default function NewProcess() {
  ...
  const handleSaveProcessWithConfirmation = async () => {
    try {
      ...
      if (result === 'PROCESS_EXISTS') {
        ...
        const mainProcess = processMovimentos && processMovimentos.length > 0 ? processMovimentos[0].process : null;
        setSavedProcess({
  ...
  const getStepTitle = () => {
    switch (currentStep) {
      case "search":
        return "Novo Processo - Buscar Processo";
      case "details":
        return "Novo Processo - Detalhes";
      case "manual":
        return "Novo Processo - Cadastro Manual";
      case "confirmation":
  ...
  return <div className="container mx-auto py-8 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          {currentStep === "search" && <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-500 hover:text-gray-900" onClick={() => setShowBatchImport(true)}>
        ...
        {currentStep === "search" && <Card className="p-6">
            <CardHeader className="px-0 pt-0 py-[6px]">
        ...
        {currentStep === "details" && processMovimentos && processMovimentos.length > 0 && <Card className="p-6">
            ...
            <CardContent className="px-0 pb-0">
              <ProcessModeDetails processMovimentos={processMovimentos} importProgress={importProgress} importComplete={importComplete} onSave={handleSaveProcessWithConfirmation} onCancel={handleCancel} onImportAnother={handleCreateAnother} handleProcessSelect={processSelectHandler} isLoading={isLoading} />
            </CardContent>
          </Card>}
        ...
        {currentStep === "confirmation" && <Card className="p-6">
            ...
            <CardFooter className="flex justify-center gap-4 px-0">
              <Button variant="outline" onClick={() => navigate('/processes')} className="min-w-[120px]">
