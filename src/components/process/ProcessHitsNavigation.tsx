...
import { ChevronLeft, ChevronRight, CalendarDays, Bell, FileText, Gavel, Users, FileSearch } from "lucide-react";
...
export function ProcessHitsNavigation({
  ...
  return <div className="w-full flex flex-col min-h-[600px]">
      <div className="space-y-3 px-2 md:px-0 flex-1 min-h-[400px]">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3 px-1 w-full">
          <div className="flex items-center gap-3 flex-wrap flex-1 min-w-[200px]">
            ...
            {totalHits > 0 && <div className="flex items-center gap-1">
                <Button variant="outline" size="icon" onClick={handlePreviousHit} className="h-8 w-8">
                  <ChevronLeft className="h-4 w-4" />
                </Button>
                <span className="text-sm mx-2">
                  {internalHitIndex + 1} / {totalHits}
                </span>
                <Button variant="outline" size="icon" onClick={handleNextHit} className="h-8 w-8">
        ...
        {currentHit && <div className="bg-white rounded-lg p-2 md:p-3 space-y-2 md:space-y-3 mb-3 mx-0 md:mx-0 overflow-visible">
            ...
            {currentHit.assuntos && currentHit.assuntos.length > 0 && <div className="mt-2 md:mt-3 pt-2 md:pt-3 border-t border-gray-100 px-1">
                <h5 className="text-[11px] md:text-xs font-medium text-gray-500 mb-1 md:mb-2">Assuntos</h5>
                <div className="flex flex-wrap gap-1 md:gap-2">
                  {currentHit.assuntos.map((assunto: {
              nome: string;
              codigo?: string;
            }, index: number) => <Badge key={index} variant="outline" className="text-xs min-w-[100px] md:min-w-0">
                      {assunto.nome}
        ...
        {currentHit && <div className="px-0 md:px-0">
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <div className="w-full pb-1 md:pb-2">
                <TabsList className="mb-1 flex flex-wrap gap-1 min-h-[32px] md:min-h-[40px] px-1 md:px-2 py-1 md:py-1.5 w-full bg-white">
                    <TabsTrigger value="eventos" className="text-[10px] md:text-xs px-2 md:px-3 py-1 md:py-1.5 flex items-center gap-1 flex-shrink-0 flex-grow
                      bg-[rgb(46_48_146)] hover:bg-[rgb(36_38_136)] text-white border border-[rgb(36_38_136)] hover:border-[rgb(26_28_126)]">
                    <CalendarDays className="h-4 w-4" />
                    <span>Eventos</span>
                  </TabsTrigger>
                  <TabsTrigger value="intimacoes" className="text-[10px] md:text-xs px-2 md:px-3 py-1 md:py-1.5 flex items-center gap-1 flex-shrink-0 flex-grow
                  ...
                  <TabsTrigger value="documentos" className="text-[10px] md:text-xs px-2 md:px-3 py-1 md:py-1.5 flex items-center gap-1 flex-shrink-0 flex-grow
                  ...
                  <TabsTrigger value="partes" className="text-[10px] md:text-xs px-2 md:px-3 py-1 md:py-1.5 flex items-center gap-1 flex-shrink-0 flex-grow
                      bg-[rgb(22_163_74)] hover:bg-[rgb(12_153_64)] text-white border border-[rgb(12_153_64)] hover:border-[rgb(2_143_54)]">
                    <Users className="h-4 w-4" />
                    <span>Partes</span>
                  </TabsTrigger>
                  <TabsTrigger value="decisao" className="text-[10px] md:text-xs px-2 md:px-3 py-1 md:py-1.5 flex items-center gap-1 flex-shrink-0 flex-grow
                      bg-[rgb(254_195_11)] hover:bg-[rgb(244_185_1)] text-black border border-[rgb(244_185_1)] hover:border-[rgb(234_175_0)]">
                    <Gavel className="h-4 w-4" />
                    <span>Decisão</span>
                  </TabsTrigger>
                    <TabsTrigger value="inteiro-teor" className="text-[10px] md:text-xs px-2 md:px-3 py-1 md:py-1.5 flex items-center gap-1 flex-shrink-0 flex-grow
              ...
              <div className="border rounded-md p-2 md:p-3 mt-1 mb-2 md:mb-4 bg-white min-h-[600px] md:min-h-[800px] flex flex-col md:flex-row md:gap-4 overflow-visible px-0 my-0 py-0">
                <div className="overflow-y-auto w-full md:flex-1 px-1">
                  <TabsContent value="eventos" className="mt-0 pt-0">
                    <ProcessMovements processId={processId} hitId={currentHit.id} filter={{
                  ascending: false
                }} />
                    
                    <div className="mt-6">
                      <h3 className="text-lg font-semibold mb-4">Linha do Tempo do Processo</h3>
                      {hits.length > 0 ? <ProcessTimeline processId={processId} hits={hits} movements={hits.flatMap(hit => hit.movements || [])} /> : <p className="text-sm text-gray-500">Nenhuma movimentação para exibir na timeline</p>}
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="decisao" className="mt-0 pt-0">
                    <ProcessDecisions processId={processId} hitId={currentHit.id} />
                  </TabsContent>
                  ...
                  <TabsContent value="inteiro-teor" className="mt-0 pt-0">
                    <ProcessDocuments processId={processId} hitId={currentHit.id} />
                  </TabsContent>
                </div>
              </div>
            </Tabs>
