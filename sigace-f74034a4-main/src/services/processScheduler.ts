...
export async function checkScheduledUpdates(): Promise<void> {
  ...
  try {
    // Busca processos com agendamento ativo e que precisam ser verificados
    const { data: rawProcesses, error } = await supabase
      .from('processes')
      .select('*')
      .eq('schedule_config->enabled', true)
      .lt('schedule_config->nextCheck', now.toISOString());

    if (error) {
      console.error('Erro ao buscar processos agendados:', error);
      return;
    }
